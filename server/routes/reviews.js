...
reviewsRouter.post('/create', requireUser, async ( req, res, next ) => {
    ...
    if (!rating || !title || !comment) {
        return res.send({success: false, message: "Title, rating, and comment are required fields"});
    }
    ...
    try {
        ...
        if (!hasPurchased) {
            return res.send({success: false, message: "A product can only be reviewed by users who have purchased it"})
        } else if (rating > 5 || rating < 1) {
            return res.send({success: false, message: "Ratings must be a number between 1 and 5"});
        } else if (hasPurchased){
            const newReview = await createReview( {userId, productId, title, rating, comment});
            return res.send({success: true, message: "Review Created!", review: newReview});
        }
    } catch (error) {
